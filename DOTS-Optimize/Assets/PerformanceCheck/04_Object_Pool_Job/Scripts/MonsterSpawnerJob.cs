...
namespace Object_Pool_Job
{
    public class MonsterSpawnerJob : MonoBehaviour
    {
        ...
        void Start()
        {
            ...
            monsterPositions = new NativeArray<Vector3>(numMonsters, Allocator.Persistent);
            monsterAngles = new NativeArray<float>(numMonsters, Allocator.Persistent);
            monsterDistances = new NativeArray<float>(numMonsters, Allocator.Persistent);

            StartCoroutine(CreateMonster());
        }
        
        public NativeArray<Vector3> GetMonsterPositions()
        {
        ...
        private IEnumerator CreateMonster()
        {
            ...
            // 플레이어 위치를 가져옴
            NativeArray<Vector3> playerPositions = playerSpawner.GetPlayerPositions();
                ...
                do
                {
                    ...
                    for (int j = 0; j < playerPositions.Length; j++)
                    {
                        if (math.distance(playerPositions[j], 
                                new Vector3(distance * math.cos(angle), 0, distance * math.sin(angle))) < 2.0f)
                        {
                            isOverlapping = true;
                            break;
                        }
            ...
            for (int i = 0; i < numMonsters; i++)
            {
                Instantiate(monsterPrefab, monsterPositions[i], Quaternion.identity, transform);
            }
