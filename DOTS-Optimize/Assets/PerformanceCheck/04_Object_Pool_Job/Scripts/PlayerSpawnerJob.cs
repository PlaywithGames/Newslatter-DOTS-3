...
namespace Object_Pool_Job
{
    public class PlayerSpawnerJob : MonoBehaviour
    {
        ...
        public JobHandle playerJobHandle;
        ...
        void Start()
        {
            ...
            playerJobHandle = job.Schedule(numPlayers, 64); // Adjust the batch size as needed
            playerJobHandle.Complete();
            
            for (int i = 0; i < numPlayers; i++)
            {
                Instantiate(playerPrefab, playerPositions[i], Quaternion.identity, transform);
            }
            
            Debug.Log("[TA][PlayerSpawnerJob] playerPositions length : " + playerPositions.Length);
